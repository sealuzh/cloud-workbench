  ...
  def create
    ...
    if @metric_observation.save
      respond_to do |format|
        format.json { render json: @metric_observation, status: :created } # 201
        format.html { render json: @metric_observation, status: :created } # 201
      end
    else
      respond_to do |format|
        format.json { head :internal_server_error }
        format.html { render html: 'Could not create metric observation', status: :internal_server_error }
      end
    end
  end
